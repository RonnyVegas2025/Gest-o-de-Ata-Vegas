...
export default function AdminPage() {
  ...
  async function loadAll() {
    ...
    const { data: d, error: e1 } = await supabase
      .from('departamentos')
      .select('id, nome, sigla, cor, ativo')
      .order('nome')

    const { data: t, error: e2 } = await supabase
      .from('tipos_documento')
      .select('id, nome, prefixo, requer_aprovacao, ativo')
      .order('nome')

    const { data: u, error: e3 } = await supabase
      .from('perfis')
      .select('id, nome, email, perfil, ativo, departamentos(sigla)')
      .order('nome')

    if (e1) setErro('Erro ao carregar departamentos: ' + e1.message)
    if (e2) setErro('Erro ao carregar tipos: ' + e2.message)
    if (e3) setErro('Erro ao carregar usuários: ' + e3.message)
  ...
  async function addDepto() {
    ...
    const { error } = await supabase.from('departamentos').insert({
      nome: newDepto.nome,
      sigla: newDepto.sigla.toUpperCase(),
    })
  ...
  async function addTipo() {
    ...
    const { error } = await supabase.from('tipos_documento').insert({
      nome: newTipo.nome,
      prefixo: newTipo.prefixo.toUpperCase(),
      requer_aprovacao: newTipo.requer_aprovacao,
    })
    if (error) { alert('Erro: ' + error.message); return }
  ...
  async function deleteDepto(id: string) {
    if (!confirm('Confirmar exclusão?')) return
    await supabase.from('departamentos').update({ ativo: false }).eq('id', id)
    loadAll()
  }

  async function deleteTipo(id: string) {
    if (!confirm('Confirmar exclusão?')) return
    await supabase.from('tipos_documento').update({ ativo: false }).eq('id', id)
    loadAll()
  }
  ...
  return (
    <AppShell>
      ...
      <div style={{ padding:'22px 26px' }}>
        ...
        {erro && (
          <div style={{ background:'#FCEBEB', color:'#A32D2D', padding:'10px 16px', borderRadius:10, marginBottom:16, fontSize:12.5 }}>
            {erro}
          </div>
        )}

        {loading && (
          <div style={{ textAlign:'center', padding:40, color:'#AEADC0', fontSize:13 }}>Carregando...</div>
        )}
          ...
          <div className="card">
            <div style={{ padding:'14px 18px', borderBottom:'1px solid #EAEAF0' }}>
              ...
              <div style={{ fontSize:11.5, color:'#AEADC0', marginTop:2 }}>
                {deptos.length} departamento{deptos.length !== 1 ? 's' : ''} cadastrado{deptos.length !== 1 ? 's' : ''}
              </div>
            </div>

            {deptos.length === 0 && (
              <div style={{ padding:'24px 18px', textAlign:'center', color:'#AEADC0', fontSize:13 }}>
                Nenhum departamento ainda. Adicione o primeiro abaixo.
              </div>
            )}

            {deptos.map(d => (
              <div key={d.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 18px', borderBottom:'1px solid #F4F4F8' }}>
                <div style={{ width:36, height:36, borderRadius:9, background:(d.cor ?? '#7F77DD')+'22', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                  <span style={{ fontSize:11, fontWeight:800, color: d.cor ?? '#7F77DD' }}>{d.sigla}</span>
                </div>
                <div>
                ...
                <div style={{ marginLeft:'auto' }}>
                  <button
                    onClick={() => deleteDepto(d.id)}
                    style={{ width:26, height:26, borderRadius:7, border:'1px solid #EAEAF0', background:'#fff', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', color:'#AEADC0' }}
            ...
            <div style={{ padding:'14px 18px', background:'#FAFAFA', borderTop:'1px solid #EAEAF0' }}>
              <div style={{ display:'flex', gap:8 }}>
                <input
                  ...
                  value={newDepto.nome}
                  onChange={e => setNewDepto(p => ({ ...p, nome: e.target.value }))}
                  style={{ flex:1 }}
                />
                <input
                  className="form-input"
                  placeholder="Sigla"
                  value={newDepto.sigla}
                  onChange={e => setNewDepto(p => ({ ...p, sigla: e.target.value }))}
                  style={{ width:90 }}
                  maxLength={5}
                />
        ...
        {!loading && tab === 'tipos' && (
          <div className="card">
            ...
            {tipos.length === 0 && (
              <div style={{ padding:'24px 18px', textAlign:'center', color:'#AEADC0', fontSize:13 }}>
                Nenhum tipo ainda. Adicione o primeiro abaixo.
              </div>
            )}
            ...
            <div style={{ padding:'14px 18px', background:'#FAFAFA', borderTop:'1px solid #EAEAF0' }}>
              <div style={{ display:'flex', gap:8 }}>
                ...
                <input className="form-input" placeholder="Prefixo" value={newTipo.prefixo} onChange={e => setNewTipo(p => ({ ...p, prefixo: e.target.value }))} style={{ width:90 }} maxLength={5}/>
            ...
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              ...
              <tbody>
                {usuarios.map(u => {
                  ...
                  const initials = (u.nome ?? 'U').split(' ').map((n: string) => n[0]).join('').slice(0,2).toUpperCase()
                  return (
                    <tr key={u.id} style={{ borderBottom:'1px solid #F4F4F8' }}>
                      ...
                      <td style={{ padding:'11px 16px' }}>
                        <span style={{ display:'inline-block', padding:'2px 8px', borderRadius:20, fontSize:10.5, fontWeight:700, background:rc.bg, color:rc.color }}>{u.perfil}</span>
                      </td>
                      <td style={{ padding:'11px 16px' }}>
                        {(u as any).departamentos?.sigla
                          ? <span className="dept-chip">{(u as any).departamentos.sigla}</span>
                          : <span style={{ color:'#AEADC0', fontSize:12 }}>—</span>}
                      </td>
                      <td style={{ padding:'11px 16px' }}>
                        <span className={u.ativo ? 'badge-aprovado' : 'badge-rascunho'}>{u.ativo ? 'Ativo' : 'Inativo'}</span>
                      </td>
                    </tr>
                  )
                })}
      ...
      {showModal && (
        <div
          style={{ position:'fixed', inset:0, background:'rgba(44,42,64,.45)', zIndex:200, display:'flex', alignItems:'center', justifyContent:'center' }}
        ...
        >
          <div style={{ background:'#fff', borderRadius:16, padding:24, width:'100%', maxWidth:480, boxShadow:'0 8px 40px rgba(44,42,64,.18)' }}>
            ...
            <div style={{ fontSize:12, color:'#AEADC0', marginBottom:20 }}>
              A senha padrão será <strong>Vegas@2026!</strong> — o usuário deve alterar no primeiro acesso.
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:14 }}>
              <div>
                <label style={{ display:'block', fontSize:11.5, fontWeight:700, color:'#5A5878', marginBottom:6 }}>Nome completo *</label>
                <input className="form-input" placeholder="Ex: João Silva" value={newUser.nome} onChange={e => setNewUser(p => ({ ...p, nome: e.target.value }))}/>
              </div>
              <div>
                <label style={{ display:'block', fontSize:11.5, fontWeight:700, color:'#5A5878', marginBottom:6 }}>E-mail *</label>
                <input className="form-input" type="email" placeholder="joao@vegas.com.br" value={newUser.email} onChange={e => setNewUser(p => ({ ...p, email: e.target.value }))}/>
              </div>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginBottom:20 }}>
              <div>
                <label style={{ display:'block', fontSize:11.5, fontWeight:700, color:'#5A5878', marginBottom:6 }}>Perfil *</label>
