            ...
            <thead>
              <tr style={{ background:'#FAFAFA' }}>
                {['Documento','Tipo','Departamento','Responsável','Data','Status','Ações'].map(h => (
                  <th key={h} style={{ fontSize:10.5, fontWeight:700, color:'#AEADC0', textAlign:'left', padding:'10px 16px', borderBottom:'1px solid #EAEAF0', textTransform:'uppercase', letterSpacing:'.06em', whiteSpace:'nowrap' }}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {(docs ?? []).map((d: any) => (
                <tr key={d.id} style={{ borderBottom:'1px solid #F4F4F8' }}>
                  ...
                  <td style={{ padding:'11px 16px', fontSize:12, color:'#9E9DB5', fontWeight:600 }}>
                    {d.tipos_documento?.nome ?? '—'}
                  </td>
                  <td style={{ padding:'11px 16px' }}>
                    <span className="dept-chip">{d.departamentos?.sigla ?? '—'}</span>
                  </td>
                  <td style={{ padding:'11px 16px', fontSize:12 }}>
                    {d.perfis?.nome ?? '—'}
                  </td>
                  <td style={{ padding:'11px 16px', fontSize:11, color:'#AEADC0', whiteSpace:'nowrap' }}>
                  ...
                  <td style={{ padding:'11px 16px' }}>
                    <a
                      ...
                      style={{ width:26, height:26, borderRadius:7, border:'1px solid #EAEAF0', background:'#fff', display:'inline-flex', alignItems:'center', justifyContent:'center', color:'#AEADC0', textDecoration:'none' }}
                      title="Visualizar"
                    >
              ...
              {(docs?.length ?? 0) === 0 && (
                <tr>
                  <td colSpan={7} style={{ padding:'32px 16px', textAlign:'center', color:'#AEADC0', fontSize:13 }}>
                    Nenhum documento encontrado.{' '}
                    <a href="/documentos/novo" style={{ color:'#7F77DD', fontWeight:700 }}>Criar novo</a>
                  </td>
                </tr>
              )}
            </tbody>
